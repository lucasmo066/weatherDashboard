    ...
    var response = fetch("https://api.openweathermap.org/data/2.5/onecall?lat="+lat+"&lon="+lon+"&units=imperial&exclude=minutely,hourly,alerts&appid=3818ca5ac2a56508daf02a0f6528796f")
        .then(function(response){
            if(response.ok){
                ...
                alert("Unable to retrieve five day forecast");
            }
        });
...
var display5Day = function(forecast){
    ...
    var dailyforecast = document.createElement("h3");
    dailyforecast.className="col-12 lookAhead"
    dailyforecast.textContent="Look Ahead:";
    fiveDayEl.append(dailyforecast);

    for(var i=1; i<6; i++){
        var fiveDayForecast = forecast.daily[i];
